// LeetCode: 419. Battleships in a Board
// 419-Battleships-in-a-Board.cpp
// Created by Victor Rogulenko
// [2022-05-19 18:03]	

// Given an m x n matrix board where each cell is a battleship 'X' or 
// empty '.', return the number of the battleships on board.

// Battleships can only be placed horizontally or vertically on board. 
// In other words, they can only be made of the shape 1 x k (1 row, k columns) 
// or k x 1 (k rows, 1 column), where k can be of any size. At least one 
// horizontal or vertical cell separates between two battleships 
// (i.e., there are no adjacent battleships).

// Follow up: Could you do it in one-pass, using only O(1) extra memory 
// and without modifying the values board?

// m == board.length
// n == board[i].length
// 1 <= m, n <= 200
// board[i][j] is either '.' or 'X'.


// MAJOR IDEA: Count the "first" cells of the battleships only

#include <vector>

class Solution {
public:
    int countBattleships(const std::vector<std::vector<char>>& board) {
        int nrows = board.size();
        int ncols = board[0].size();
        int result = 0;
        
        for (int idx = 0; idx < nrows; ++idx) {
            for (int jdx = 0; jdx < ncols; ++jdx) {
                if (board[idx][jdx] == 'X') {
                    if ((idx == 0 || board[idx-1][jdx] == '.') &&
                        (jdx == 0 || board[idx][jdx-1] == '.')) {
                        ++result;
                    }
                }
            }
        }
            
        return result;
    }
};